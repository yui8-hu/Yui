...
async function sendAnimu(sock, chatId, message, type) {
    ...
    async function convertMediaToSticker(mediaBuffer, isAnimated) {
        ...
        const json = {
            ...
            'sticker-pack-publisher': '𝒀𝒖𝒊 𝒃𝒐𝒕 ☕︎',
            'emojis': ['🎌']
        };
    ...
    if (data.link) {
        ...
        try {
            await sock.sendMessage(
                chatId,
                { image: { url: link }, caption: `⎯⎯ 𝒀𝒖𝒊 𝒃𝒐𝒕 ☕︎\n\n✨ أنمي: ${type}` },
                { quoted: message }
            );
            return;
        } catch {}
    ...
    await sock.sendMessage(
        chatId,
        { text: '╭─────── ☕︎ ───────╮\n  ❌ فشل في جلب تفاعل الأنمي، حاول مرة أخرى.\n╰─────── 𝜗𝜚 ───────╯' },
        { quoted: message }
    );
}
...
async function animeCommand(sock, chatId, message, args) {
    ...
    const supported = [
        'nom', 'poke', 'cry', 'pat', 'hug', 'wink', 'face-palm', 'quote'
    ];
    ...
    try {
        if (!sub) {
            const helpMsg = `𓍢ִ໋🌷͙֒  𝒀𝒖𝒊 𝒃𝒐𝒕 ☕︎\n\n` +
                `╭━━━ ⋆｡°✩🕯️✩°｡⋆ ━━━╮\n` +
                `  ✨ *تفاعلات الأنمي المتاحة:*\n` +
                `╰━━━ ⋆｡°✩🍵✩°｡⋆ ━━━╯\n\n` +
                `𓂃 ☕︎ • *.أنمي حضن* (hug)\n` +
                `𓂃 ☕︎ • *.أنمي رأس* (pat)\n` +
                `𓂃 ☕︎ • *.أنمي بكاء* (cry)\n` +
                `𓂃 ☕︎ • *.أنمي غمزة* (wink)\n` +
                `𓂃 ☕︎ • *.أنمي كف* (face-palm)\n` +
                `𓂃 ☕︎ • *.أنمي اقتباس* (quote)\n` +
                `𓂃 ☕︎ • *.أنمي أكل* (nom)\n` +
                `𓂃 ☕︎ • *.أنمي وخز* (poke)\n\n` +
                `─── ִ ࣪ ˖ ☕︎ ִ ׄ ───\n` +
        ...
        if (!supported.includes(sub)) {
            await sock.sendMessage(chatId, { text: `╭─────── 🕯️ ───────╮\n  ❌ النوع غير مدعوم: ${sub}\n  يرجى كتابة *.أنمي* لمعرفة الأنواع المتاحة ☕︎\n╰─────── 𝜗𝜚 ───────╯` }, { quoted: message });
            return;
        }
        ...
        await sock.sendMessage(chatId, { text: '╭─────── ☕︎ ───────╮\n  ❌ حدث خطأ أثناء جلب ملصق الأنمي.\n╰─────── 𝜗𝜚 ───────╯' }, { quoted: message });
    }
}
