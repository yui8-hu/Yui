...
async function stickerCommand(sock, chatId, message) {
    ...
    if (!mediaMessage) {
        await sock.sendMessage(chatId, { 
            text: '⚠️ يرجى الرد على صورة أو فيديو بالأمر *.ملصق* (أو *.sticker*)، أو إرسال الصورة/الفيديو وكتابة الأمر في الوصف!',
            contextInfo: {
                forwardingScore: 999,
                isForwarded: true,
    ...
    try {
        ...
        if (!mediaBuffer) {
            await sock.sendMessage(chatId, { 
                text: '❌ فشل تحميل الوسائط، يرجى المحاولة مرة أخرى.',
                contextInfo: {
                    forwardingScore: 999,
                    isForwarded: true,
                    forwardedNewsletterMessageInfo: {
        ...
        // Create metadata with custom rights
        const json = {
            'sticker-pack-id': crypto.randomBytes(32).toString('hex'),
            'sticker-pack-name': settings.packname || '𝑲.𝑵.𝑯 ┆ 𝑳𝑈𝑆𝑰𝐴',
            'sticker-pack-publisher': settings.author || 'Lusia Bot',
            'emojis': ['✨']
        };
        ...
        // Final safety: if still too large, make a tiny pass
        if (isAnimated && finalBuffer.length > 900 * 1024) {
            try {
                ...
                if (fs.existsSync(tempOutput3)) {
                    ...
                    const json2 = {
                        'sticker-pack-id': crypto.randomBytes(32).toString('hex'),
                        'sticker-pack-name': settings.packname || '⎯⎯ 𝒀𝒖𝒊 𝒃𝒐𝒕 ☕︎',
                        'sticker-pack-publisher': settings.author || '⎯⎯ 𝒀𝒖𝒊 𝒃𝒐𝒕 ☕︎',
                        'emojis': ['✨']
                    };
        ...
        await sock.sendMessage(chatId, { 
            text: '❌ حدث خطأ أثناء إنشاء الملصق! يرجى المحاولة لاحقاً.',
            contextInfo: {
                forwardingScore: 999,
                isForwarded: true,
                forwardedNewsletterMessageInfo: {
